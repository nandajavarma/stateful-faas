...
\begin{center}
  \textbf{Sommario}\par
\end{center}
Serverless Computing è una piattaforma emergente come offerta di servizi
dove il fornitore di servizi cloud gestisce e alloca
risorse necessarie per mantenere in esecuzione l'applicazione. Ciò consente allo sviluppatore di concentrarsi sullo sviluppo dell'applicazione
e non sulla manutenzione del server. Oltre a caricare il provisioning e
la manutenzione del server, l'elaborazione serverless riduce anche lo spreco di risorse
aumentando e diminuendo l'allocazione a seconda del carico e del
configurazioni. Gli utenti pagano solo per le risorse che sono state utilizzate da
applicazione risparmiando così enormi costi operativi sulla loro infrastruttura
ospitando.
...
\label{sec:orgec9be7d}
\paragraph{Statelessness}
\label{sec:org60ae276}
...
being longer and there won't be much notable difference in the money spent \hyperref[ref:7]{7}
...
\label{sec:orgb6c7260}
\paragraph{Faas + Microservices}
\label{sec:org8fe8bf3}
...
50\% parallelism was noted \hyperref[ref:17]{17}. The reason for this is noted mostly to
be the following:
\begin{itemize}
...
operations \hyperref[ref:16]{16}. Having said that, the implementation usually involves
...
\label{sec:org4604332}
It is quite curious to note from the survey of 2018 done by serverless \hyperref[ref:16]{16} that the
...
\label{sec:org566e66e}

\paragraph{Logs}
\label{sec:org8a8f926}
...
\subsection{Tools}
\label{sec:org9588075}
\subsubsection{Container Orchestration}
\label{sec:org1f07485}
...
FaaS-flow can orchestrate a pipeline with long running and short running
function performing ETL jobs without having to orchestrate them manually or
maintaining a separate application. Faas-flow ensures the execution order of
...
\section{Conclusion}
\label{sec:orge502b64}
\section{References}
\label{sec:orgb9399bd}
\begin{enumerate}
